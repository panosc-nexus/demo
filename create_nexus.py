...
from extra_geom import AGIPD_1MGeometry as Adipd


def collect_frames(files: list, h5path: str = '/data/data') -> list:
    """Collect data frames from separate h5 files

    Parameters
    ----------
    files: list
        list of file names to collect images from. Each file is supposed to have
        exactly one frame inside
    h5path: str
        the path to the image data within the h5-hierarchy
    """
    frames = []
    for f in files:
        pass
...
class Agipd2nexus:
    ...
    def __init__(self, args):
        ...
        self.params_from_conf(args.conf)
        self.output_file_name = args.output + '_master_from_defs.h5'
        self.stat = Counter()

    def params_from_conf(self, filename: str = None) -> dict:
        """ Collects arguments from both the config"""
        if filename and os.path.isfile(filename):
            with open(filename, 'r') as fd:
                try:
                    self.params = yaml.load(fd)
    ...
    def get_root_path(self, tree_elem: etree) -> List[str]:
        ...
        root_path = ['/' + '/'.join(path).replace('NX', '') for path in
                     product(*ancestor_list)]  # concatenate the values
        return root_path
    ...
    def __init__(self, args):
        ...
        if self.params['detector_distance'] is None:
            ...
            if self.hierarchy.detector_distance:
                ...
                raise Exception("Detector distance is undefined! You should set it either in `.phil` or in `.geom` files, "
            ...
            for module_num in range(self.n_modules):  # iterate modules within a quadrant
                ...
                for asic_num in range(self.n_asics):  # iterate asics within a module
                    ...
                    det_field_rules[full_m_name('fast_pixel_direction')] = NexusElement(
                        full_path=full_m_name('fast_pixel_direction'), value=[pixel_size], dtype='f',
                        nxtype=NxType.field,
                    ...
                    det_field_rules[full_m_name('slow_pixel_direction')] = NexusElement(
                        full_path=full_m_name('slow_pixel_direction'), value=[pixel_size], dtype='f',
                        nxtype=NxType.field,
                        attrs={'depends_on': t_path + f'AXIS_D0Q{quad}M{module_num}A{asic_num}',
...
if __name__ == '__main__':
    ap = ArgumentParser(description='Create NeXus file from AGIPD data')
    ap.add_argument('-c', '--conf', help='Configuration file in YAML format')
    ap.add_argument('-o', '--output', help='Output file')
    ap.add_argument('-g', '--geom', help='CrystFEL geometry file')
    args = ap.parse_args(sys.argv[1:])
    nexus_helper = Ruleset(args)
